...
class Wechat {
	...
	/* 企业付款 需要证书*/
	const PAY_TRANSFERS_ORDER	  = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';
	/* 下载对账单 */
	...
	/**
	 * 企业付款 需要证书操作 //byhzd 20160305
	 * @param $openid 用户openid
	 * @param $transfersId 商户订单号
	 * @param $user_name 收款用户真实姓名
	 * @param $amount 金额（分）
	 * @param $desc 企业付款操作说明信息
	 * @param $ip 调用接口的机器Ip地址
	 */
	public function transfers($openid, $transfersId, $amount, $desc, $ip, $user_name = '') {
		$params['mch_appid']          = $this->appid;
		$params['mchid']         = $this->mch_id;
		$params['nonce_str']      = self::_getRandomStr();
		$params['partner_trade_no']   = $transfersId;
		$params['openid']     = $openid;
		$params['check_name']  = 'NO_CHECK';
		if (!empty($user_name)) {
			$params['re_user_name']      = $user_name; //收款用户真实姓名。 如果check_name设置为FORCE_CHECK或OPTION_CHECK，则必填用户真实姓名
		}
		...
		$params['sign']           = self::_getOrderMd5($params);
		$data = self::_array2Xml($params);
		$data = $this->http(self::PAY_TRANSFERS_ORDER, $data, 'POST', true); 
		return self::_extractXml($data);
	}
